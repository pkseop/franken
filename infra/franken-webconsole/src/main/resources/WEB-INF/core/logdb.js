...
var Query = function(jobj) {
	...
	function createQuery(querystr) {
		socket.send("kr.co.future.logdb.msgbus.LogQueryPlugin.createQuery", { "query": querystr }, function(m) {
	...
	function startQuery(queryId) {
		socket.send("kr.co.future.logdb.msgbus.LogQueryPlugin.startQuery",
			{
				id: queryId,
				offset: 0,
				limit: pagesize,
	...
	function getResult(id, offset, limit, trigger) {
		socket.send("kr.co.future.logdb.msgbus.LogQueryPlugin.getResult",
			{
				id: id,
				offset: offset,
	...
	this.stop = function(callback) {
		...
		socket.send("kr.co.future.logdb.msgbus.LogQueryPlugin.stopQuery", { "id": that.activeId() }, function(m) {
			if(!!callback) {
				callback();
			}
		});
	...
	function removeQuery(item, callback) {
		socket.send("kr.co.future.logdb.msgbus.LogQueryPlugin.removeQuery", { "id": item.activeId() }, function(m) {
			if(!!callback) {
				callback();
			}
		});
...
var logdbManager = (function() {
	...
	function getQueries(callback) {
		socket.send("kr.co.future.logdb.msgbus.LogQueryPlugin.queries", {}, function(m) {
			if(m.isError) {
