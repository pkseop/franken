		...
		if (session.has("enc_key")) {
			try {
				...
				if (header.containsKey("iv") && body.containsKey("data")) {
					...
					ByteBuf ivBuf = null, ivDecoded = null, bufBuf = null, bufDecoded = null;
					try {
						ivBuf = Unpooled.wrappedBuffer(header.get("iv").toString().getBytes());
						ivDecoded = Base64.decode(ivBuf);
						byte[] iv = ByteUtil.asArray(ivDecoded);
						
						bufBuf = Unpooled.wrappedBuffer(data.getBytes());
						bufDecoded = Base64.decode(bufBuf);
						
						byte[] buf = ByteUtil.asArray(bufDecoded);
