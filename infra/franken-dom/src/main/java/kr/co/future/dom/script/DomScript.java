...
import kr.co.future.dom.model.HostExtension;
import kr.co.future.dom.model.HostType;
import kr.co.future.dom.model.OrganizationUnit;
import kr.co.future.dom.model.User;
import kr.co.future.dom.model.Vendor;
...
//	}
    ...
    @ScriptUsage(description = "create test users", arguments = {@ScriptArgument(name = "number of test user", type = "int", description = "number of test user")})
    public void createTestUsers(String[] args) {
        int countUser = Integer.parseInt(args[0]);

        User user = null;

        context.println("Create test user start...");
        context.println("-------------------------");
        for (int i = 0; i < countUser; i++) {
            ...
            user.setPassword("08421f0c3e41e403271883a523c5ff13d865db05");

            userApi.createUser("localhost", user, true);

            context.println("test" + i + " created.");
        }

        context.println("Create test user end...");
    }
    
    public void modifyAdminAccount(String[] args) {
    	try {
    		User user = userApi.getMasterUser();
    		if(user == null) {
    			context.println("couldn't find admin account");
    			return;
    		}
    		
    		context.print("enter password: ");
    		String pw = context.readPassword();
    		boolean result = userApi.verifyPassword("localhost", user.getLoginName(), pw);
    		if(result) {
    			context.print("enter new account name: ");
        		String newLoginName = context.readLine();
        		
        		boolean res = userApi.updateLoginName(user.getLoginName(), newLoginName);
        		if(res)
        			context.println("succeeded!");
        		else
