...
import kr.co.future.confdb.ConfigEntry;
...
public class FileManifestCodec implements CustomCodec {
	...
	public Object decode(ByteBuffer bb) {
		...
		for (int i = 0; i < colCount; i++) {
			...
			int configCount = 0;
			if (version == 2)
				configCount = bb.getShort() & 0xffff;
			else if (version == 3)
				configCount = bb.getInt(); 
	...
	public int lengthOf(Object value) {
		...
		try {
			for (String colName : manifest.getCollectionNames()) {
				// col id (2b), name len (2b), name, config count (4b) 
				total += 8 + colName.getBytes("utf-8").length;
