...
;    MessageBox MB_OK "$R1"
	...
	${Endif}
;	MessageBox MB_OK "$R0"
	ExecWait `"$R0$\" //IS//$\"Kraken_Core$\" --Install=$\"$R0$\" --Description=$\"Kraken Core 1.7.0$\" --Jvm=$\"$R1$\" --Startup=auto --StartMode=jvm --Classpath=$\"$INSTDIR\core\kraken-core-1.7.0-package.jar$\" --StartClass=kr.co.future.core.main.Kraken --StartMethod=windowsService --StartParams=start --StopMode=jvm --StopClass=kr.co.future.core.main.Kraken --StopMethod=windowsService --StopParams=stop --LogPath=$\"$INSTDIR\log$\" --StdError=auto --StdOutput=auto --StartPath=$\"$INSTDIR"`
