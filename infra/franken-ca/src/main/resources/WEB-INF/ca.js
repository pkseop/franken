CAManager = function() {
	...
	this.onstart = function(pid, args) {
		...
		function getRootCertificates(callback) {
			channel.send(1, 'kr.co.future.ca.msgbus.CaPlugin.getRootCertificates', {},
				function(resp) {
					var certlist = [];
		...
		function getCertificates(node, callback) {
			...
			channel.send(1, 'kr.co.future.ca.msgbus.CaPlugin.getCertificates', 
				{
					"ca_common_name": cn
				},
				function(resp) {
		...
		function getCertificate(cn, alias, password, callback, error_callback) {
			channel.send(1, 'kr.co.future.ca.msgbus.CaPlugin.getCertificate', 
				{
					"ca_common_name": cn,
					"key_alias": alias,
		...
		function getPfxFile(cn, alias, password) {
			channel.send(1, 'kr.co.future.ca.msgbus.CaPlugin.getPfxFile',
				{
					"ca_common_name": cn,
					"key_alias": alias,
					"key_password": password 
		...
		function issueRootCertificate() {
			...
			btnSave.handler = function() {
				channel.send(1, 'kr.co.future.ca.msgbus.CaPlugin.issueRootCertificate',
		...
		function issueCertificate() {
			...
			btnSave.handler = function() {
				channel.send(1, 'kr.co.future.ca.msgbus.CaPlugin.issueCertificate',
					CreatePropertyGrid.getSource(),
					function(resp) {
						tree.enable();
						getRootCertificates();
				...
				this.items = [
					{
						...
						loader: new Ext.tree.TreeLoader({
							directFn: function(nodeid, callback, node) {
								console.log(node) // �̰� �ٷ� kraken.js���� TreeLoader�� override �ؼ� �̾Ƴ� �κ�
