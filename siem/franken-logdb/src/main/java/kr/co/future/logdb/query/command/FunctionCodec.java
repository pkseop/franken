...
import kr.co.future.logdb.sort.SortCodec;

import kr.co.future.codec.CustomCodec;
import kr.co.future.codec.EncodedStringCache;
import kr.co.future.codec.EncodingRule;
import kr.co.future.codec.TypeMismatchException;
import kr.co.future.codec.UnsupportedTypeException;
...
public class FunctionCodec implements CustomCodec {
	...
	private List<Object> getDatas(byte code, Function value) {
		...
		if (code == (byte) 128) {
			...
			datas.add(((Function.DistinctCount) value).getResult());
	...
	public Object decode(ByteBuffer bb) {
		...
		if (code == (byte) 128) {
			...
			((Function.DistinctCount) func).setResult((Long) EncodingRule.decode(bb));
