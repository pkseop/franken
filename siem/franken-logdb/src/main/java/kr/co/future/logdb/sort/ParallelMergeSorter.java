 ...
 */
package kr.co.future.logdb.sort;
...
//		executor.submit(new FlushWorker(buffered));
		new Thread(new FlushWorker(buffered)).start();
	}
...
//		executor.shutdown();
	...
	private Run mergeAll(List<Partition> partitions) throws IOException {
		...
		int size = tasks.size();
		mergeBarrier = new CyclicBarrier(size + 1);
		for (PartitionMergeTask task : tasks) {
			merges.add(task);
			//Using both ExecutorService and CyclicBarrer can cause a problem.
			//The problem is the process stayed in waiting state forever, if the number of tasks are more than 8.
//			executor.submit(new MergeWorker(task));
			new Thread(new MergeWorker(task, size)).start();
		}
	...
	private class MergeWorker implements Runnable {
		...
		public MergeWorker(PartitionMergeTask task, int size) {
			this.task = task;
			this.size = size;
		}
		...
		public void run() {
			try {
				...
				try {
					logger.trace("kraken logdb: number of waiting [{}]", mergeBarrier.getNumberWaiting());
					mergeBarrier.await();
				} catch (InterruptedException e) {
				} catch (BrokenBarrierException e) {
