...
import kr.co.future.logstorage.LogTableNotFoundException;
...
public class LogTableRegistryImpl implements LogTableRegistry {
	...
	private void loadTableMappings() {
		...
		if (col == null) {
			...
			ConfigCollection before = db.getCollection("kr.co.future.logstorage.engine.LogTableSchema");
			if (before != null) {
				...
				try {
					...
					xact.commit("kraken-logstorage", "migration from collection kr.co.future.logstorage.engine.LogTableSchema");
					db.dropCollection("kr.co.future.logstorage.engine.LogTableSchema");
				} catch (Throwable e) {
					xact.rollback();
	...
	@Deprecated
	@Override
	public kr.co.future.logstorage.TableMetadata getTableMetadata(int tableId) {
		...
		return new kr.co.future.logstorage.TableMetadata(tableId, tableName, m);
	}
