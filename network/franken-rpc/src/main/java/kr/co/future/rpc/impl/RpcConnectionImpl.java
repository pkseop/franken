...
import kr.co.future.rpc.RpcAsyncCallback;
import kr.co.future.rpc.RpcAsyncResult;
...
import kr.co.future.rpc.RpcConnectionState;
...
import kr.co.future.rpc.RpcSessionState;
...
public class RpcConnectionImpl implements RpcConnection, RpcSessionEventCallback {
	...
	public void bind(String name, RpcService service) {
		logger.trace("kraken rpc: binding {}:{} to {}", new Object[] { channel.hashCode(), name, service.getClass().getName() });
	...
	public void close() {
		...
		logger.trace("kraken rpc: connection closed id={}, peer={}", channel.hashCode(), getRemoteAddress());
	}
	...
	public String toString() {
		return String.format("id=%d, peer=(%s, %s), trusted level=%s, ssl=%s", channel.hashCode(), getPeerGuid(),
