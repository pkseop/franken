...
public class Request implements HttpServletRequest {
	...
	public Request(ChannelHandlerContext ctx, HttpRequest req, HttpContent httpContent) {
		this.ctx = ctx;
		this.req = req;
		...
		if(httpContent != null) {
			ByteBuf b = httpContent.content();
	        byte[] bufferBytes = new byte[b.readableBytes()];
	        b.getBytes(b.readerIndex(), bufferBytes);
	        this.is = new RequestInputStream(new ByteArrayInputStream(bufferBytes, 0, b.readableBytes()));
		}
		...
		if (req.getMethod().equals(HttpMethod.POST)) {
			if (!(contentType != null && contentType.equals("application/octet-stream"))) {
				if(httpContent != null) {
			        ...
			        byte[] bufferBytes = new byte[b.readableBytes()];
			        b.getBytes(b.readerIndex(), bufferBytes);
			        String body = new String(bufferBytes, b.readerIndex(), b.readableBytes(), Charset.forName("utf-8"));
					setParams(body);
				}
			}
		}
	...
	private void setChannel(ChannelHandlerContext ctx) {
		...
		setAttribute("netty.channel", ctx.channel());
	}
	...
	private void parseCookies(HttpRequest req) {
		...
		for (int i = 0; i < cs.size(); i++) {
			...
			try {
				...
				logger.error("error occurred while making cookie instance [{}]", e);
				continue;
			}
	...
	private void setParams(String params) {
		...
		for (String param : params.split("&")) {
			...
			if (pos > 0) {
				...
				StringBuffer outBuffer = new  StringBuffer(encodedValue);
				value = urlDecode(outBuffer);
	...
	//To handle this exception => URLDecoder: Illegal hex characters in escape (%) pattern
	public static String urlDecode(StringBuffer outBuffer) {
