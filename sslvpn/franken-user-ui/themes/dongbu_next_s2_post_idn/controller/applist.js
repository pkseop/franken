...
function (ctlSsl, ctlConn, ctlPage, ctlReset, $l, $util) {
    ...
    var OnCompleted_RequestUserInfo = function (obj) {
        ...
        /////////////
        var accountExpiryAlert = obj.account_expiry_alert;
        var accountExpiryRemain = obj.account_expiry_remain / 60 / 60;
        var accountExpriyDay = 0;

        if (accountExpiryRemain > 24)
            accountExpriyDay = accountExpiryRemain / 24;

        if (accountExpiryRemain > 0 && accountExpiryAlert > accountExpriyDay) {
            if (accountExpriyDay > 0) {
                var date = parseInt(accountExpriyDay);
                var sMsg = "계정만료 " + date + "일 전";
                $("#lbl_expirydatecount").text(sMsg);
            }
            else {
                var sMsg = "계정만료 " + accountExpiryRemain + "시간 전";
                $("#lbl_expirydatecount").text(sMsg);
            }
        }
        else {
            $("#area_expirydatecount").hide();
        }

        if (obj.start_at != "" && obj.expire_at != "") {
            $("#lbl_StartAt").text(obj.start_at);
            $("#lbl_ExpireAt").text(obj.expire_at);
        }
        else {
            $("#area_expirydate").hide();
        }
